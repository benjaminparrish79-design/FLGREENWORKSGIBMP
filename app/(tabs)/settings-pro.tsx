...
export default function SettingsScreen() {
  ...
  const [proPackage, setProPackage] = useState<Package | null>(null);
  ...
  const loadSubscription = useCallback(async () => {
    try {
      setLoading(true);
      const [sub, det, pkg] = await Promise.all([
        getSubscriptionStatus(),
        getSubscriptionDetails(),
        getProPackage(),
      ]);
            ...
            >
              ...
              {!details.isPro && (
                <View className="gap-2">
                  ...
                  <Text className="text-xs text-muted">✓ Custom reports</Text>
                </View>
              )}
            </View>
          )}
